...
src = repo / 'app'
dst = Path('/tmp/teenz-ota-1195-final')
zip_path = Path('/tmp/teenz-1.1.195-final.zip')
...
assert 'runtime-fixes-1.1.195.js' in index
assert 'runtime-fixes-1.1.195.css' in index
assert 'assets/index-GemFix1184.js' in index
assert not any(f'index-GemFix{v}' in index for v in ('1185', '1186', '1187', '1188', '1189'))
runtime = (dst / 'runtime-fixes-1.1.195.js').read_text(encoding='utf-8')
...
for bridge in ['installResetProgressBridge', 'installReadingGateCloseBridge', 'installHomeBackupDismissBridge', 'installHomeBackupCardPresentation', 'installNativePhotoInputTapBridge', 'applyReaderAudioPolish']:
...
assert '[data-loc="client/src/pages/Profile.tsx:1141"]' in (dst / 'runtime-fixes-1.1.195.css').read_text(encoding='utf-8')
assert '[data-loc="client/src/pages/Profile.tsx:1142"]' in (dst / 'runtime-fixes-1.1.195.css').read_text(encoding='utf-8')
css = (dst / 'runtime-fixes-1.1.195.css').read_text(encoding='utf-8')
assert 'tb-reader-audio-panel' in css
assert 'tb-reader-audio-panel__content' in css
assert 'tb-meme-share-dialog' in css
assert 'tb-meme-notice' in css
for bridge in ['installMemeShareSaveBridge', 'openMemeShareDialog', 'saveMemeFile']:
    assert bridge in runtime, bridge
...
shutil.copy2(zip_path, ota_dir / '1.1.195.zip')
manifest = {'version': '1.1.195', 'url': 'https://teens-bible-94271.web.app/ota/1.1.195.zip', 'checksum': sha256, 'size': size}

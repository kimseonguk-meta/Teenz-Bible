...
src = repo / 'app'
dst = Path('/tmp/teenz-ota-1186-final')
zip_path = Path('/tmp/teenz-1.1.186-final.zip')
...
assert 'runtime-fixes-1.1.186.js' in index
assert 'runtime-fixes-1.1.186.css' in index
assert 'assets/index-GemFix1184.js' in index
assert 'index-GemFix1185' not in index
assert 'index-GemFix1186' not in index

runtime = (dst / 'runtime-fixes-1.1.186.js').read_text(encoding='utf-8')
...
assert 'installResetProgressBridge' in runtime
assert 'installReadingGateCloseBridge' in runtime
assert 'installHomeBackupDismissBridge' in runtime
assert 'installHomeBackupCardPresentation' in runtime
...
shutil.copy2(zip_path, ota_dir / '1.1.186.zip')
manifest = {
    'version': '1.1.186',
    'url': 'https://teens-bible-94271.web.app/ota/1.1.186.zip',
    'checksum': sha256,
    'size': size,
}

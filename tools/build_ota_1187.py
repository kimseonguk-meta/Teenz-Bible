...
src = repo / 'app'
dst = Path('/tmp/teenz-ota-1187-final')
zip_path = Path('/tmp/teenz-1.1.187-final.zip')
...
assert 'runtime-fixes-1.1.187.js' in index
assert 'runtime-fixes-1.1.187.css' in index
assert 'assets/index-GemFix1184.js' in index
assert 'index-GemFix1185' not in index
assert 'index-GemFix1186' not in index
assert 'index-GemFix1187' not in index
runtime = (dst / 'runtime-fixes-1.1.187.js').read_text(encoding='utf-8')
...
shutil.copy2(zip_path, ota_dir / '1.1.187.zip')
manifest = {'version': '1.1.187', 'url': 'https://teens-bible-94271.web.app/ota/1.1.187.zip', 'checksum': sha256, 'size': size}

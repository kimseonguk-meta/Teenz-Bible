...
## 1.1.192 Ranking 신고 안전 규칙

Ranking 유저 모달의 기본 액션은 Cheer·More actions·Close다. Block 버튼은 현재 active runtime에서 제거되었다. More actions 아래에는 Report this member만 노출된다. Report를 누르면 Are you sure? 확인 단계가 먼저 나오며, Continue 후 신고 사유를 선택하고 선택적으로 최대 500자의 추가 설명을 입력해야 Submit report가 활성화된다. 실제 저장은 기존 `safetyReports.json` 경로를 사용하며 reason과 details를 기록한다. QA에서는 실제 신고 전송을 실행하지 않는다.
...
주요 탐색·Bible AI·Bible Map·Reader·Bible 목록·Ranking·Gem Store·Profile 흐름은 감사 범위에서 대체로 정상이다. 다음 네 항목은 아직 별도 QA가 필요하다.
...
5. Bible Meme Share·Save는 1.1.195 공개 browser 검증을 통과했지만 실제 iPad에서 native share sheet와 Photos 저장 결과를 확인해야 한다.
6. Ranking Report는 공개 웹에서 Block 제거, More actions, 확인 단계, 사유 선택, 선택적 설명, Submit 활성화 조건까지 검증했다. 실제 신고 전송과 관리자 검토 흐름은 별도 테스트 계정으로 확인해야 한다.
...
Report는 `safetyReports`를 사용한다. 현재 Ranking UI에서는 Block 버튼을 제거했지만, 기존 `blocks` 데이터와 backend helper가 저장소에 남아 있을 수 있으므로 후속 작업에서 임의로 Rules나 historical data를 삭제하지 않는다. Report의 확인·사유 입력·제출은 실제 iPad와 Galaxy PWA에서 touch interaction까지 검증한다.

## 다음 작업 시작 순서

1. GitHub `main`에서 1.1.195 문서·패키지 커밋을 pull하고 공개 manifest checksum을 대조한다.
2. 이 문서, `README.md`, `AI_CONTINUITY_GUIDE.md`, `HANDOFF.md`, `ENVIRONMENT.md`, `NEW_DEVELOPER_CHECKLIST.md`, `native-ios/README.md`, `docs/APP_STORE_CONNECT_METADATA.md`, `docs/CONVERSATION_DECISIONS.md`, `reports/reader-audio-qa-1193.txt`, `reports/reader-audio-qa-1194.txt`, `reports/meme-share-save-qa-1195.txt`, 최종 감사 보고서를 읽는다.

...
export default function Leaderboard() {
  ...
  const [cachedMembers, setCachedMembers] = useState<Record<string, LeaderboardMember[]>>({});
  ...
  // Load data - only after auth is ready, with cache and skeleton per fix #4
  const loadData = useCallback(async (force = false) => {
    if (!currentUid) return; // Wait for auth
    const cacheKey = `${mainTab}-${selectedGroupCode || groupCode}-${timeFilter}-${sortBy}`;
    // Use cached data if available to avoid flicker on tab switch.
    // Return early on cache hit: refetching here re-triggers this callback
    // via cachedMembers state update and causes an infinite download loop.
    if (!force && cachedMembers[cacheKey]) {
      setMembers(cachedMembers[cacheKey]);
      // Serve from cache without refetching; a refetch here would loop.
      setLoading(false);
      return;
    } else if (!cachedMembers[cacheKey]) {
      // Only show loading spinner on first load or when no cache
    ...
    try {
      ...
      // Count/error logging per existing requirement
      console.log(`[Leaderboard] fetched ${raw?.length ?? 0} members for ${cacheKey}, uid=${currentUid}`);
      if (!Array.isArray(raw)) {
        console.warn("[Leaderboard] fetch result not array, using empty");
        raw = [];
      }
      const filtered = filterByTime(raw, timeFilter);
      const sorted = sortMembers(filtered, sortBy);
      setMembers(sorted);
      setCachedMembers(prev => ({ ...prev, [cacheKey]: sorted }));
      ...
      if (err?.message === "PERMISSION_DENIED") {
        setError("Permission denied — check RTDB rules for /groups");
      } else {
  ...
  }, [mainTab, selectedGroupCode, timeFilter, sortBy, groupCode, currentUid, cachedMembers, members.length]);
  ...
  return (
    <div className="px-4 pt-6 space-y-4 pb-4">
      ...
      {/* Sort Tabs — hidden when My Groups tab has no groups (empty state above says it all) */}
      {!(mainTab === "mygroups" && userGroups.length === 0) && (
        <>
      <div id="lb-tabs" className="flex gap-1.5 justify-center flex-wrap">
        {SORT_TABS.map((tab) => (
          <button
            key={tab.key}
      ...
      {/* Loading / Error / Content with skeleton per fix #4 */}
      {initialLoading ? (
        <div className="space-y-3 py-4">
          <div className="flex gap-2 justify-center">
            <div className="h-8 w-16 rounded-full bg-purple-900/30 animate-pulse" />
            <div className="h-8 w-16 rounded-full bg-purple-900/20 animate-pulse" />
            <div className="h-8 w-16 rounded-full bg-purple-900/20 animate-pulse" />
          </div>
          <div className="flex justify-center gap-3 pt-4">
            <div className="w-14 h-14 rounded-full bg-purple-900/20 animate-pulse" />
            <div className="w-[72px] h-[72px] rounded-full bg-yellow-900/20 animate-pulse" />
            <div className="w-14 h-14 rounded-full bg-purple-900/20 animate-pulse" />
          </div>
          <div className="space-y-2">
            {[0,1,2,3,4].map(i => (
              <div key={i} className="h-16 rounded-xl bg-white/[0.03] border border-purple-500/10 animate-pulse" />
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-12 gap-2">
          ...
          <p className="text-gray-300 text-sm font-medium">No members yet</p>
          <p className="text-gray-500 text-xs text-center px-6">
            {mainTab === "mygroups"
              ? "This group has no members with activity yet. Start reading to appear!"
              : "No active members found. Be the first to start reading!"}
          </p>
          <button
            onClick={() => loadData(true)}
            className="mt-2 px-4 py-2 rounded-full bg-purple-600/30 border border-purple-500/30 text-purple-300 text-xs font-medium active:scale-95"
          >
            🔄 Refresh
          </button>
        </div>
        ...
        <>
          ...
          {loading && members.length > 0 && (
            <div className="flex justify-center py-1">
              <span className="text-[10px] text-purple-400/70 animate-pulse">Refreshing...</span>
            </div>
          )}
          ...
          {top3.length >= 3 && (
            <div className="flex items-end justify-center gap-3 pt-4 pb-2 min-w-0">
              {/* 2nd place */}
              <div
                className="flex flex-col items-center cursor-pointer active:scale-95 transition-transform min-w-0"
              ...
              >
                <div className="relative flex-shrink-0">
                  ...
                  <div className={`w-14 h-14 rounded-full bg-[rgba(15,5,40,0.8)] flex items-center justify-center overflow-hidden flex-shrink-0 shadow-[0_0_15px_rgba(96,165,250,0.3)] ${!getFrameClass(top3[1]?.equippedFrame) ? "border-2 border-blue-400/50" : ""}`}>
                    <MemberAvatar member={top3[1]} size="md" showFrame />
                  </div>
                </div>
                <p className="text-white text-xs font-bold mt-2 max-w-[120px] truncate">{top3[1]?.nickname}</p>
              ...
              {/* 1st place */}
              <div
                className="flex flex-col items-center -mt-4 cursor-pointer active:scale-95 transition-transform min-w-0"
              ...
              >
                ...
                <div className="relative flex-shrink-0">
                  ...
                  <div className={`flex items-center justify-center overflow-hidden flex-shrink-0 shadow-[0_0_20px_rgba(234,179,8,0.3)] ${!getFrameClass(top3[0]?.equippedFrame) ? "border-2 border-yellow-500/70" : ""}`} style={{width: '72px', height: '72px', borderRadius: '50%', background: 'rgba(15,5,40,0.8)'}}>
                    <MemberAvatar member={top3[0]} size="lg" showFrame />
                  </div>
                </div>
                <p className="text-white text-sm font-bold mt-2 max-w-[120px] truncate">
                  {top3[0]?.nickname}
              ...
              >
                <div className="relative flex-shrink-0">
                  ...
                  <div className={`w-14 h-14 rounded-full bg-[rgba(15,5,40,0.8)] flex items-center justify-center overflow-hidden flex-shrink-0 shadow-[0_0_15px_rgba(180,83,9,0.3)] ${!getFrameClass(top3[2]?.equippedFrame) ? "border-2 border-amber-600/50" : ""}`}>
                    <MemberAvatar member={top3[2]} size="md" showFrame />
                  </div>
                </div>
                <p className="text-white text-xs font-bold mt-2 max-w-[120px] truncate">{top3[2]?.nickname}</p>
            ...
            {rest.map((member, idx) => {
              ...
              return (
                <div
                  ...
                  className={`neon-card p-4 flex items-center gap-3 rounded-xl transition-all cursor-pointer active:scale-[0.98] min-w-0 overflow-hidden ${
                    isMe
                      ? "!border-purple-500 shadow-[0_0_18px_rgba(148,64,255,0.45)]"
                      : ""
                  }`}
                >
                  <span className="text-base font-bold text-gray-400 w-7 text-center flex-shrink-0 ml-3">{rank}</span>
                  <div className={`w-10 h-10 rounded-full bg-purple-900/50 flex items-center justify-center overflow-hidden flex-shrink-0 ${memberFrameClass || "border border-purple-500/30"}`}>
                    {member.profilePhotoUrl ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <p className={`font-medium text-sm truncate max-w-[120px] ${isMe ? "text-red-400" : "text-white"}`}>
                        {member.nickname || "Anonymous"}
                        {isMe && " (You)"}
                      </p>
                      {member.joinedAt && (Date.now() - member.joinedAt < 7 * 24 * 60 * 60 * 1000) && (
                        <span className="text-[9px] text-gray-500 flex-shrink-0">NEW</span>
                      )}
                      {mainTab === "global" && renderGroupBadgeInline(member.groupCode)}
                    </div>
                    <p className="text-gray-400 text-xs truncate max-w-[180px] leading-relaxed">{getDisplayValue(member, sortBy)}</p>
                  </div>
                </div>
              );
            })}

...
function getQuizHistory(): QuizHistoryEntry[] {
  return safeParseJSON<QuizHistoryEntry[]>("quizHistory", []);
}
  ...
  return (
    <div className="min-h-screen pb-24 px-4 pt-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate("/profile")} className="text-purple-300 active:scale-95 transition-transform">
          ← Back
        </button>
        ...
        <div className="space-y-4">
          ...
          <div className="neon-card p-4">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-purple-300">Quizzes taken</span>
              <span className="text-white font-bold">{weeklyProgress.total}</span>
            </div>
            <div className="flex justify-between text-sm mt-1">
              <span className="text-purple-300">Correct answers</span>
              <span className="text-green-400 font-bold">{weeklyProgress.correct}</span>
            </div>
            <div className="flex justify-between text-sm mt-1">
              <span className="text-purple-300">Weekly accuracy</span>
          ...
          <div className="neon-card p-4">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-purple-300">Best streak</span>
              ...
              <div className="space-y-2">
                {mostMissed.map(({ book, accuracy: acc, total }) => (
                  <div key={book} className="flex items-center justify-between">
                    <span className="text-purple-200 text-sm">{book}</span>
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-2 bg-purple-900/50 rounded-full overflow-hidden">
                        <div 
        ...
        <div className="space-y-3">
          {stats.total === 0 ? (
            <div className="neon-card p-6 text-center">
              ...
              <p className="text-purple-200 mt-3">No quiz data yet! Start reading chapters to take quizzes.</p>
            </div>
          ) : (
            <>
              <p className="text-purple-300 text-xs mb-2">Books you've been quizzed on:</p>
              {Object.entries(bookStats)
                .sort((a, b) => b[1].total - a[1].total)
                .map(([book, s]) => {
                  const acc = Math.round((s.correct / s.total) * 100);
                  return (
                    <div key={book} className="neon-card p-3 flex items-center justify-between">
                      <div>
                        <p className="text-white font-bold text-sm">{book}</p>
                        <p className="text-purple-300 text-xs">{s.correct}/{s.total} correct</p>
                      </div>
                      <div className="flex items-center gap-2">
      ...
      {tab === "history" && (
        <div className="space-y-2">
          {recentHistory.length === 0 ? (
            <div className="neon-card p-6 text-center">
              <span className="text-4xl">📋</span>
              <p className="text-purple-200 mt-3">No quiz history yet! Start reading to take quizzes.</p>
            </div>
          ) : (
            <>
              <p className="text-purple-300 text-xs mb-2">Last 20 quizzes:</p>
              {recentHistory.map((entry, i) => (
                <div key={i} className="neon-card p-3 flex items-center justify-between">
...
function StatCard({ label, value, emoji, color }: { label: string; value: string; emoji: string; color?: string }) {
  ...
  return (
    <div className="neon-card p-4 text-center">
      ...
      <p className="text-purple-300 text-xs mt-1">{label}</p>
    </div>
  );
}

...
import { auth, googleProvider, db, ref, get, set, popupResolver } from "./firebase";
...
async function performGoogleSignIn() {
  if (isNativePlatform()) {
    ...
    // Web: use popup with explicit resolver (root cause fix for popupRedirectResolver)
    return await signInWithPopup(auth, googleProvider, popupResolver);
  }
}

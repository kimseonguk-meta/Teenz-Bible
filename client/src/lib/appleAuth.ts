...
import { auth, appleProvider, db, ref, get, set, popupResolver } from "./firebase";
...
export async function linkOrSignInWithApple(): Promise<LinkResult> {
  ...
  try {
    ...
    if (isNativePlatform()) {
      ...
      const result = await signInWithPopup(auth, appleProvider, popupResolver);
      await immediateSyncToFirebase();
...
async function linkAnonymousToApple(): Promise<LinkResult> {
  ...
  try {
    ...
    if (!isNativePlatform()) {
      // Web: use popup with explicit resolver (root cause fix)
      try {
        const popupResult = await signInWithPopup(auth, appleProvider, popupResolver);

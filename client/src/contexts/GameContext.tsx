...
import { recordChapterReadToday, reconcileReminders } from "@/lib/readingReminders";
...
function loadState(): GameState {
  ...
  return {
    ...
    totalXP: (() => {
      const v = parseInt(localStorage.getItem("totalXP") || "0");
      return Number.isNaN(v) ? 0 : v;
    })(),
    gems: teensBible.gems || 0,
    ownedPets: safeParseJSON<string[]>("ownedPets", ["Faithy Cat"]),
    equippedPet: localStorage.getItem("equippedPet") || "Faithy Cat",
    ownedThemes: safeParseJSON<string[]>("ownedThemes", ["Midnight Black"]),
    equippedTheme: localStorage.getItem("equippedTheme") || "Midnight Black",
    ownedFrames: safeParseJSON<string[]>("ownedFrames", ["Basic"]),
    equippedFrame: localStorage.getItem("equippedFrame") || "Basic",
    badges: safeParseJSON<string[]>("badges", ["First Step"]),
    watchedVideos: safeParseJSON<string[]>("watchedVideos", [] as string[]),
    settingsOpen: false,
    editNameOpen: false,
...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const getChaptersRead = useCallback((book: string): number[] => {
    return safeParseJSON<number[]>(`chaptersRead_${book}`, []);
  }, []);
  ...
  const markChapterRead = useCallback((book: string, chapterNum: number) => {
    ...
    if (!read.includes(chapterNum)) {
      ...
      // Evening reminder bookkeeping: mark today as read, cancel tonight's reminders
      recordChapterReadToday();
  ...
  const getWatchedVideos = useCallback((): string[] => {
    return safeParseJSON<string[]>("watchedVideos", []);
  }, []);

(() => {
  ...
  // Route/lazy transitions can leave old App.tsx Suspense fallbacks above an already
  // rendered page shell. This watchdog only hides stale DOM panels; it never swaps the
  // React bundle while running, so it is safe for both web and native.
  const installLoadingFallbackGuard = () => {
    if (!document.body) return;
    const root = document.getElementById('root');
    if (!root || root.__tbLoadingFallbackGuard) return;
    const restorePanel = (panel) => {
      if (!panel?.classList?.contains('tb-native-stale-loading')) return;
      panel.classList.remove('tb-native-stale-loading');
    ...
    const hidePanel = (panel) => {
      panel.classList.add('tb-native-stale-loading');
    ...
    let scheduled = false;
    const hideStaleFallbacks = () => {
      scheduled = false;
      // Only target the actual App.tsx fallback wrapper. Never climb to the outer
      // h-screen app shell, because hiding that shell causes a black screen during navigation.
      root.querySelectorAll('.tb-native-stale-loading').forEach(restorePanel);
      const loadingSpans = Array.from(root.querySelectorAll('span')).filter((node) => node.textContent?.trim() === 'Loading...');
      const panels = loadingSpans.map((span) => {
        let node = span.parentElement;
        while (node && node !== root) {
          const classText = String(node.className || '');
          if (/h-64/.test(classText) && /flex/.test(classText) && /items-center/.test(classText)) return node;
          node = node.parentElement;
        }
        return null;
      }).filter(Boolean);
      panels.forEach(restorePanel);
      // During a real route transition one fallback is expected and must remain visible.
      // Only hide older duplicates when there are at least two exact fallback wrappers.
      if (panels.length > 1) panels.slice(0, -1).forEach(hidePanel);
    };
    const scheduleHideStaleFallbacks = () => {
      if (scheduled) return;
      scheduled = true;
      window.requestAnimationFrame(hideStaleFallbacks);
    };
    root.__tbLoadingFallbackGuard = true;
    root.__tbLoadingFallbackObserver = new MutationObserver(scheduleHideStaleFallbacks);
    root.__tbLoadingFallbackObserver.observe(root, { childList: true, subtree: true });
    root.__tbLoadingFallbackInterval = window.setInterval(hideStaleFallbacks, 500);
    window.addEventListener('focus', scheduleHideStaleFallbacks, { passive: true });
    window.addEventListener('pageshow', scheduleHideStaleFallbacks, { passive: true });
    document.addEventListener('visibilitychange', scheduleHideStaleFallbacks, { passive: true });
    window.addEventListener('teenzBibleAppStateChange', scheduleHideStaleFallbacks, { passive: true });
    try {
      const appPlugin = window.Capacitor?.Plugins?.App;
      if (appPlugin?.addListener) appPlugin.addListener('appStateChange', scheduleHideStaleFallbacks);
    } catch (_) { /* Optional Capacitor App plugin; never block web startup. */ }
    hideStaleFallbacks();
  };
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', installLoadingFallbackGuard, { once: true });
  else installLoadingFallbackGuard();
  ...
  const openRankingCrewCreator = () => {
    ...
    submit?.addEventListener('click', async () => {
      ...
      try {
        ...
        const core = await import('/assets/index-GemFix1179.js');
  ...
  const preloadStoreModule = () => {
    ...
    window.__TEENZ_STORE_PREFETCH__ = import('/assets/Store-GemFix1179.js')
      .then(() => true)
      .catch((error) => {

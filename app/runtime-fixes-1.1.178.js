(() => {
  ...
  const installLoadingFallbackGuard = () => {
    ...
    const hideStaleFallbacks = () => {
      scheduled = false;
      const pageShell = root.querySelector('.tb-page');
      ...
      const panels = loadingSpans.map((span) => {
        ...
        while (node && node !== root) {
          ...
          if (/min-h-screen|h-screen/.test(classText) && /flex/.test(classText)) return node;
          node = node.parentElement;
        }
        return span;
      });
      panels.forEach(restorePanel);
      const stalePanels = pageShell ? panels.filter((panel) => panel !== pageShell && !pageShell.contains(panel)) : panels.slice(0, -1);
      stalePanels.forEach(hidePanel);
    };
  ...
  const openRankingCrewCreator = () => {
    ...
    submit?.addEventListener('click', async () => {
      ...
      try {
        ...
        const core = await import('/assets/index-GemFix1178.js');
  ...
  const preloadStoreModule = () => {
    ...
    window.__TEENZ_STORE_PREFETCH__ = import('/assets/Store-GemFix1178.js')
      .then(() => true)
      .catch((error) => {

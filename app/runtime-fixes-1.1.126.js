(() => {
  ...
  const openSecureAccountDeleteDialog = async () => {
    ...
    dialog.innerHTML = `
      <section class="tb-secure-account-delete__card" role="dialog" aria-modal="true" aria-labelledby="tb-delete-title">
        ...
        <p class="tb-secure-account-delete__identity">${escapeHtml(session?.name || 'Your Teenz Bible account')}</p>
    ...
    const syncConfirmationState = () => {
      const ready = typed?.value.trim().toUpperCase() === ACCOUNT_DELETE_TYPED_CONFIRMATION && acknowledgement?.checked;
  ...
  const installRankingActionPortal = () => {
    ...
    portal.innerHTML = '<div class="tb-ranking-action-portal__primary"><button type="button" data-tb-portal-close>Close</button><button type="button" data-tb-portal-cheer>⚔️ Cheer</button></div><div class="tb-ranking-action-portal__safety"><button type="button" data-tb-portal-report>Report</button><button type="button" data-tb-portal-block>Block</button></div>';
  ...
  const removeBlockingProfilePhotoNudge = () => {
    ...
    candidates.forEach((node) => {
      ...
      if (/Add a Profile Photo!/i.test(text) && /Let's go|Maybe later/i.test(text)) {
        ...
        overlay.setAttribute('data-tb-photo-nudge-hidden', 'true');
      }
    });
  };
  ...
  const installDirectProfilePhotoChooser = () => {
    ...
    if (photoOverlay && photoDialog) {
      // Preserve React ownership of the dialog, but hide only the sibling Profile
      // content and bottom navigation. Their independent stacking contexts cannot
      // then bleed through the photo sheet on Android PWA rendering.
      isolatePhotoModalForeground(photoOverlay);
      // Keep the modal inside React's root for event delegation, but remove any
      // transformed app-shell containing block that would displace fixed children.
      ...
      photoDialog.style.setProperty('position', 'fixed', 'important');
      photoDialog.style.setProperty('top', '50%', 'important');
      photoDialog.style.setProperty('left', '50%', 'important');
      photoDialog.style.setProperty('right', 'auto', 'important');
      photoDialog.style.setProperty('bottom', 'auto', 'important');
      ...
      photoDialog.style.setProperty('transform', 'translate(-50%, -50%)', 'important');
      ...
      // Some mobile shells establish a non-viewport fixed containing block even
      // when computed transforms are identity. Measure that offset and cancel it.
      const overlayRect = photoOverlay.getBoundingClientRect();
      photoOverlay.style.setProperty('left', `${-overlayRect.x}px`, 'important');
      photoOverlay.style.setProperty('top', `${-overlayRect.y}px`, 'important');
      photoOverlay.style.setProperty('right', 'auto', 'important');
      photoOverlay.style.setProperty('bottom', 'auto', 'important');
      photoOverlay.style.setProperty('width', `${window.innerWidth}px`, 'important');
      photoOverlay.style.setProperty('height', `${window.innerHeight}px`, 'important');

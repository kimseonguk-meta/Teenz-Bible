(() => {
  ...
  // Native WKWebView recovery: a failed OTA/lazy-route transition can leave one or more
  // old App.tsx Suspense fallbacks above the already-rendered page shell. Keep the
  // fallback while no page exists, but hide stale duplicates once the active .tb-page
  // is present. This is native-only and never changes the ordinary web PWA.
  const installNativeLoadingFallbackGuard = () => {
    if (!isNativePlatform() || !document.body) return;
    const root = document.getElementById('root');
    if (!root || root.__tbNativeLoadingFallbackGuard) return;
    const hideStaleFallbacks = () => {
      ...
      if (pageShell) {
        panels.forEach((panel) => {
          if (panel !== pageShell && !pageShell.contains(panel)) {
    ...
    root.__tbNativeLoadingFallbackGuard = true;
    root.__tbNativeLoadingFallbackObserver = new MutationObserver(hideStaleFallbacks);
    root.__tbNativeLoadingFallbackObserver.observe(root, { childList: true, subtree: true });
    hideStaleFallbacks();
    window.setInterval(hideStaleFallbacks, 500);
  };
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', installNativeLoadingFallbackGuard, { once: true });
  else installNativeLoadingFallbackGuard();
  ...
  const openRankingCrewCreator = () => {
    ...
    submit?.addEventListener('click', async () => {
      ...
      try {
        ...
        const core = await import('/assets/index-GemFix1177.js');
  ...
  const preloadStoreModule = () => {
    ...
    window.__TEENZ_STORE_PREFETCH__ = import('/assets/Store-GemFix1177.js')
      .then(() => true)
      .catch((error) => {

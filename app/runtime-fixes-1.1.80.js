(() => {
  ...
  const openRankingCrewCreator = () => {
    ...
    submit?.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!savedGroups.some((group) => group.groupCode === code)) savedGroups.push({ groupCode: code, joinedAt: now, role: 'admin' });
  ...
  const openCrewPicker = async () => {
    ...
    try {
      const names = await Promise.all(records.map(async (record) => {
        try {
          const response = await fetch(`https://teens-bible-94271-default-rtdb.firebaseio.com/groupMeta/${encodeURIComponent(record.groupCode)}.json`);
          const meta = response.ok ? await response.json() : null;
          return { ...record, name: meta?.name || record.groupCode };
        } catch (_) { return { ...record, name: record.groupCode }; }
      }));
  ...
  const installCrewPicker = () => {
    const candidates = Array.from(document.querySelectorAll('[data-loc*="Leaderboard"], button, [role="button"], a'))
      .filter((element) => /VIEWING CREW/i.test(element.textContent || ''))

(() => {
  ...
  const closeSafetyReportConfirmDialog = () => document.getElementById('tb-safety-report-confirm-dialog')?.remove();

  const openSafetyReportConfirmDialog = (targetUid, targetName) => {
    if (document.getElementById('tb-safety-report-confirm-dialog')) return;
    const dialog = document.createElement('div');
    dialog.id = 'tb-safety-report-confirm-dialog';
    dialog.className = 'tb-safety-report-dialog tb-safety-report-dialog--confirm';
    dialog.innerHTML = `<section class="tb-safety-report-dialog__card" role="dialog" aria-modal="true"><p class="tb-safety-report-dialog__eyebrow">SAFETY REVIEW</p><h2>Are you sure?</h2><p>You will choose a reason before anything is submitted. Reports are private and help keep the community safe.</p><div class="tb-safety-report-dialog__actions"><button type="button" data-tb-report-confirm-cancel>Cancel</button><button type="button" data-tb-report-confirm-continue>Continue</button></div></section>`;
    document.body.appendChild(dialog);
    dialog.querySelector('[data-tb-report-confirm-cancel]')?.addEventListener('click', closeSafetyReportConfirmDialog);
    dialog.addEventListener('click', (event) => { if (event.target === dialog) closeSafetyReportConfirmDialog(); });
    dialog.querySelector('[data-tb-report-confirm-continue]')?.addEventListener('click', () => {
      closeSafetyReportConfirmDialog();
  ...
  const openSafetyReportDialog = (targetUid, targetName) => {
    ...
    dialog.innerHTML = `<section class="tb-safety-report-dialog__card" role="dialog" aria-modal="true"><p class="tb-safety-report-dialog__eyebrow">SAFETY REPORT</p><h2>Report this member</h2><p>Help us understand what happened. Your report is private.</p><label>Why are you reporting this member?<select data-tb-report-reason><option value="" selected>Select a reason</option><option value="Harassment or bullying">Harassment or bullying</option><option value="Inappropriate language">Inappropriate language</option><option value="Spam or repeated messages">Spam or repeated messages</option><option value="Impersonation">Impersonation</option><option value="Something else">Something else</option></select></label><label>Add details <span>(optional)</span><textarea data-tb-report-details maxlength="500" placeholder="What happened?"></textarea></label><p class="tb-report-status" aria-live="polite"></p><div class="tb-safety-report-dialog__actions"><button type="button" data-tb-report-cancel>Cancel</button><button type="button" data-tb-report-submit disabled>Submit report</button></div></section>`;
    document.body.appendChild(dialog);
    const status = dialog.querySelector('.tb-report-status');
    const reasonSelect = dialog.querySelector('[data-tb-report-reason]');
    const submit = dialog.querySelector('[data-tb-report-submit]');
    reasonSelect?.addEventListener('change', () => { if (submit) submit.disabled = !reasonSelect.value; });
  ...
  const installRankingActionPortal = () => {
    ...
    portal.innerHTML = '<div class="tb-ranking-action-portal__heading"><span>MEMBER ACTIONS</span><button type="button" data-tb-portal-close aria-label="Close member profile">✕</button></div><button type="button" class="tb-ranking-action-portal__cheer" data-tb-portal-cheer>⚔️ <span>CHEER</span><small>Send encouragement</small></button><button type="button" class="tb-ranking-action-portal__more" data-tb-portal-more aria-expanded="false"><span>•••</span><span>More actions</span><span aria-hidden="true">⌄</span></button><div class="tb-ranking-action-portal__safety" hidden><button type="button" data-tb-portal-report>⚑&nbsp; Report this member</button><p>Safety tools are here when you need them.</p></div><div class="tb-ranking-action-portal__secondary"><button type="button" data-tb-portal-close>Close</button></div>';
    ...
    const moreButton = portal.querySelector('[data-tb-portal-more]');
    const safetyGroup = portal.querySelector('.tb-ranking-action-portal__safety');
    moreButton?.addEventListener('click', () => {
      const expanded = moreButton.getAttribute('aria-expanded') === 'true';
      moreButton.setAttribute('aria-expanded', String(!expanded));
      if (safetyGroup) safetyGroup.hidden = expanded;
      moreButton.classList.toggle('is-open', !expanded);
    });
    portal.querySelector('[data-tb-portal-report]')?.addEventListener('click', () => {
      ...
      openSafetyReportConfirmDialog(targetUid, targetName);
    });
  ...
  const clearPhotoModalIsolation = () => {
    ...
    document.querySelectorAll('.tb-photo-crop-host').forEach((host) => {
      host.classList.remove('tb-photo-crop-host');
      ['position', 'left', 'top', 'right', 'bottom', 'width', 'height', 'z-index'].forEach((property) => host.style.removeProperty(property));
    });
  };
  ...
  const installNativePhotoInputTapBridge = () => {
    if (!isNativePlatform()) return;
    const sheet = document.querySelector('[data-loc="client/src/pages/Profile.tsx:907"]');
    if (!sheet) return;
    const mappings = [
      { buttonClass: 'tb-photo-option-camera', inputLoc: 'client/src/pages/Profile.tsx:1101' },
      { buttonClass: 'tb-photo-option-gallery', inputLoc: 'client/src/pages/Profile.tsx:1120' },
    ];
    mappings.forEach(({ buttonClass, inputLoc }) => {
      const button = sheet.querySelector(`button.${buttonClass}`);
      const input = document.querySelector(`[data-loc="${inputLoc}"]`);
      if (!(button instanceof HTMLButtonElement) || !(input instanceof HTMLInputElement) || button.dataset.tbNativePhotoInputBound === 'true') return;
      button.dataset.tbNativePhotoInputBound = 'true';
  ...
  const applyReaderAudioPolish = () => {
    const panel = document.querySelector('[data-loc="client/src/pages/Bible.tsx:1983"]');
    if (!panel) return;
    panel.classList.add('tb-reader-audio-panel');
    panel.querySelector('[data-loc="client/src/pages/Bible.tsx:1985"]')?.classList.add('tb-reader-audio-progress');
    panel.querySelector('[data-loc="client/src/pages/Bible.tsx:1991"]')?.classList.add('tb-reader-audio-panel__content');
    panel.querySelector('[data-loc="client/src/pages/Bible.tsx:1992"]')?.classList.add('tb-reader-audio-panel__main');
    panel.querySelector('[data-loc="client/src/pages/Bible.tsx:1993"]')?.classList.add('tb-reader-audio-playing');
    panel.querySelector('[data-loc="client/src/pages/Bible.tsx:1997"]')?.classList.add('tb-reader-audio-speed');
    panel.querySelector('[data-loc="client/src/pages/Bible.tsx:2013"]')?.classList.add('tb-reader-audio-next');
    panel.querySelector('[data-loc="client/src/pages/Bible.tsx:2015"]')?.classList.add('tb-reader-audio-toggle');
    const playing = panel.querySelector('[data-loc="client/src/pages/Bible.tsx:1994"]');
    const part = panel.querySelector('[data-loc="client/src/pages/Bible.tsx:1995"]');
    const nextLabel = panel.querySelector('[data-loc="client/src/pages/Bible.tsx:2014"]');
    const countMatch = `${playing?.textContent || ''} ${part?.textContent || ''}`.match(/\((\d+)\s*\/\s*(\d+)\)/);
    if (playing) {
      playing.textContent = `Playing ${countMatch ? `${countMatch[1]}/${countMatch[2]}` : '1/2'}`;
      playing.setAttribute('aria-label', playing.textContent);
    }
    if (part) part.textContent = '';
    if (nextLabel) {
      nextLabel.textContent = 'Auto next';
      nextLabel.setAttribute('aria-label', 'Auto next');
    }
  };
  ...
  const installDirectProfilePhotoChooser = () => {
    ...
    const cropOverlay = document.querySelector('[data-loc="client/src/pages/Profile.tsx:1141"]');
    const cropDialog = document.querySelector('[data-loc="client/src/pages/Profile.tsx:1142"]');
    const cropIsActive = !!(cropOverlay && cropDialog && (cropDialog.querySelector('input[type="range"], button') || cropDialog.textContent?.includes('Drag to move')));
    const photoOverlay = cropIsActive ? cropOverlay : document.querySelector('[data-loc="client/src/pages/Profile.tsx:907"]');
    const photoDialog = cropIsActive ? cropDialog : document.querySelector('[data-loc="client/src/pages/Profile.tsx:914"]');
    const isCropModal = cropIsActive;
    // The sheet itself is already proof of an explicit avatar action. Tag its
    // option buttons before the userInitiated visual branch so native WebViews
    // can receive the same gesture even if the avatar event was retargeted.
    if (photoOverlay && photoDialog) {
      ...
      installNativePhotoInputTapBridge();
    }
    const userInitiated = Date.now() < profilePhotoUserIntentUntil;
    if (photoOverlay && photoDialog && userInitiated) {
      photoOverlay.classList.remove('tb-react-hidden-node');
      photoOverlay.removeAttribute('aria-hidden');
      ...
      if (isCropModal) {
        // Profile.tsx mounts the crop dialog inside a zero-size relative host.
        // Make that host the shell-sized fixed layer so the crop UI cannot sit
        // underneath the This Week/badges cards or inherit their scroll offset.
        const cropHost = photoOverlay.parentElement;
        const containingParent = cropHost?.parentElement;
        const parentRect = containingParent?.getBoundingClientRect();
        if (cropHost) {
          cropHost.classList.add('tb-photo-crop-host');
          cropHost.style.setProperty('position', 'fixed', 'important');
          cropHost.style.setProperty('left', `${desiredShellLeft - (parentRect?.left || 0)}px`, 'important');
          cropHost.style.setProperty('top', `${(shellRect?.top || 0) - (parentRect?.top || 0)}px`, 'important');
          cropHost.style.setProperty('right', 'auto', 'important');
          cropHost.style.setProperty('bottom', 'auto', 'important');
          cropHost.style.setProperty('width', `${shellWidth}px`, 'important');
          cropHost.style.setProperty('height', '100dvh', 'important');
          cropHost.style.setProperty('z-index', '2147483646', 'important');
        }
        photoOverlay.style.setProperty('position', 'absolute', 'important');
        photoOverlay.style.setProperty('inset', '0', 'important');
        photoOverlay.style.setProperty('width', '100%', 'important');
        photoOverlay.style.setProperty('height', '100%', 'important');
        ...
        photoDialog.style.setProperty('top', 'auto', 'important');
        photoDialog.style.setProperty('left', 'auto', 'important');
  ...
  const installHomeBackupCardPresentation = () => {
    ...
    card.style.setProperty('background-color', 'rgba(48, 68, 60, .74)', 'important');
    card.style.setProperty('background-blend-mode', 'multiply', 'important');
    card.style.setProperty('border', '1px solid rgba(178, 207, 185, .34)', 'important');
    card.style.setProperty('box-shadow', '0 6px 16px rgba(0, 0, 0, .22), inset 0 1px rgba(232, 244, 225, .06)', 'important');
    const closeButton = card.querySelector('[data-loc="client/src/pages/Home.tsx:257"]');
    if (!(closeButton instanceof HTMLButtonElement)) return;
    closeButton.textContent = '×';
    closeButton.style.setProperty('width', '44px', 'important');
    closeButton.style.setProperty('height', '44px', 'important');
    closeButton.style.setProperty('min-width', '44px', 'important');
    closeButton.style.setProperty('margin', '-3px -3px 0 0', 'important');
    ...
    closeButton.style.setProperty('color', 'rgba(224, 236, 221, .62)', 'important');
    ...
    closeButton.style.setProperty('font-size', '0', 'important');
  ...
  const getMemeImageUrl = (button) => {
    const overlay = button?.closest('div[class~="fixed"][class~="inset-0"][class~="z-[9999]"][class~="bg-black/95"]');
    const card = button?.closest('[data-loc="client/src/pages/Home.tsx:419"]') || document.querySelector('[data-loc="client/src/pages/Home.tsx:419"]');
    const images = [
      ...(overlay ? [...overlay.querySelectorAll('img')] : []),
      ...(card ? [...card.querySelectorAll('img')] : []),
      ...[...document.querySelectorAll('img')],
    ];
    const image = images.find((candidate) => {
      const alt = (candidate.getAttribute('alt') || '').toLowerCase();
      const src = candidate.currentSrc || candidate.src || '';
      return alt.includes('meme') || /meme|daily/i.test(src);
    }) || images.find((candidate) => candidate !== document.querySelector('img'));
    return image?.currentSrc || image?.src || '';
  };

  const showMemeNotice = (message, tone = 'success') => {
    document.querySelector('[data-tb-meme-notice]')?.remove();
    const notice = document.createElement('div');
    notice.setAttribute('data-tb-meme-notice', '1');
    notice.className = `tb-meme-notice tb-meme-notice--${tone}`;
    notice.textContent = message;
    document.body.appendChild(notice);
    window.setTimeout(() => notice.remove(), 2800);
  };

  const copyMemeLink = async (url) => {
  ...
  const fetchMemeFile = async (url) => {
    const response = await fetch(url, { mode: 'cors' });
    if (!response.ok) throw new Error(`Meme image request failed (${response.status})`);
    const blob = await response.blob();
    const type = blob.type || 'image/jpeg';
    return new File([blob], 'bible-meme.jpg', { type });
  };

  const shareMemeFile = async (url) => {
    const nativePlugins = window.Capacitor?.Plugins || {};
    const nativeShare = nativePlugins.Share;
    if (isNativePlatform() && nativeShare && typeof nativeShare.share === 'function') {
      await nativeShare.share({ title: '😂 Bible Meme', text: 'Check out this Bible meme!', url });
      return 'shared';
    }
    try {
      const file = await fetchMemeFile(url);
      const payload = { title: '😂 Bible Meme', text: 'Check out this Bible meme!', files: [file] };
      ...
      // A remote image may reject CORS; the URL share/copy path below still works.
    }
    if (typeof navigator.share === 'function') {
      await navigator.share({ title: '😂 Bible Meme', text: 'Check out this Bible meme!', url });
      return 'shared';
    }
    await copyMemeLink(url);
    return 'copied';
  };

  const saveMemeFile = async (url) => {
    const nativePlugins = window.Capacitor?.Plugins || {};
    const saveToPhotos = nativePlugins.SaveToPhotos;
    if (isNativePlatform() && saveToPhotos && typeof saveToPhotos.savePhoto === 'function') {
      try {
        await saveToPhotos.savePhoto({ url });
        return 'photos';
      } catch (_) {
        // Some iOS builds do not contain the optional SaveToPhotos plugin. Continue
        // with the native Share sheet instead of surfacing a false error toast.
      }
    }
    const nativeShare = nativePlugins.Share;
    if (isNativePlatform() && nativeShare && typeof nativeShare.share === 'function') {
      await nativeShare.share({ title: 'Save Bible Meme', text: 'Choose Save Image in the share sheet.', url });
      return 'share-sheet';
    }
    try {
      const file = await fetchMemeFile(url);
      const canShareFiles = typeof navigator.canShare !== 'function' || navigator.canShare({ files: [file] });
      if (isNativePlatform() && typeof navigator.share === 'function' && canShareFiles) {
        await navigator.share({ title: 'Save Bible Meme', text: 'Choose Save Image in the share sheet.', files: [file] });
        return 'share-sheet';
      }
      const objectUrl = URL.createObjectURL(file);
      const link = document.createElement('a');
      link.href = objectUrl;
      link.download = `bible-meme-${new Date().toISOString().split('T')[0]}.jpg`;
      link.rel = 'noopener';
      ...
      if (typeof navigator.share === 'function') {
        await navigator.share({ title: 'Save Bible Meme', text: 'Open the image and save it to Photos.', url });
        return 'share-sheet';
      }
      const link = document.createElement('a');
      link.href = url;
  ...
  const openMemeShareDialog = (url) => {
    document.querySelector('[data-tb-meme-share-dialog]')?.remove();
    const dialog = document.createElement('div');
    dialog.setAttribute('data-tb-meme-share-dialog', '1');
    dialog.className = 'tb-meme-share-dialog';
    dialog.setAttribute('role', 'dialog');
    dialog.setAttribute('aria-modal', 'true');
    dialog.setAttribute('aria-label', 'Share Bible Meme');

    const panel = document.createElement('div');
    panel.className = 'tb-meme-share-dialog__panel';
    const title = document.createElement('h2');
    title.textContent = 'Share Bible Meme';
    const subtitle = document.createElement('p');
    subtitle.textContent = 'Send this meme to your crew.';
    const preview = document.createElement('img');
    preview.src = url;
    preview.alt = 'Bible Meme preview';
    preview.className = 'tb-meme-share-dialog__preview';
    const actions = document.createElement('div');
    actions.className = 'tb-meme-share-dialog__actions';
    const nativeShare = document.createElement('button');
    nativeShare.type = 'button';
    nativeShare.className = 'tb-meme-share-dialog__primary';
    nativeShare.textContent = '📤 Share image';
    const copy = document.createElement('button');
    copy.type = 'button';
    copy.className = 'tb-meme-share-dialog__secondary';
    copy.textContent = '🔗 Copy link';
    const close = document.createElement('button');
    close.type = 'button';
    close.className = 'tb-meme-share-dialog__close';
    ...
    nativeShare.addEventListener('click', async () => {
      ...
      try {
        const result = await shareMemeFile(url);
        if (result === 'copied') showMemeNotice('Link copied!');
        else showMemeNotice('Share sheet opened!');
        remove();
      } catch (error) {
        if (error?.name !== 'AbortError') showMemeNotice('Share is ready — try Copy link.', 'info');
        nativeShare.disabled = false;
      }
    });
    copy.addEventListener('click', async () => {
      ...
      try {
        await copyMemeLink(url);
        showMemeNotice('Link copied!');
        remove();
      } catch (_) {
        showMemeNotice('Copy link was not available.', 'error');
        copy.disabled = false;
      }
    });

    actions.append(nativeShare, copy);
    panel.append(title, subtitle, preview, actions, close);
    dialog.appendChild(panel);
    document.body.appendChild(dialog);
    close.focus();
  };

  let memeShareSaveBridgeInstalled = false;
  const installMemeShareSaveBridge = () => {
    if (memeShareSaveBridgeInstalled) return;
    const intercept = (event) => {
      const target = event.target instanceof Element ? event.target.closest('[data-loc="client/src/pages/Home.tsx:471"], [data-loc="client/src/pages/Home.tsx:573"], [data-loc="client/src/pages/Home.tsx:500"], [data-loc="client/src/pages/Home.tsx:599"]') : null;
      if (!target) return;
      const url = getMemeImageUrl(target);
      if (!url) return;
      event.preventDefault();
      event.stopImmediatePropagation();
      const isShare = target.matches('[data-loc="client/src/pages/Home.tsx:471"], [data-loc="client/src/pages/Home.tsx:573"]');
      if (isShare) {
        openMemeShareDialog(url);
        return;
      }
      void saveMemeFile(url).then((result) => {
        if (result === 'photos') showMemeNotice('Saved to Photos! 📥');
        else if (result === 'share-sheet') showMemeNotice('Choose Save Image in the share sheet.');
        else if (result === 'opened') showMemeNotice('Image opened — save it to Photos.');
        else showMemeNotice('Meme saved! 📥');
      }).catch((error) => {
        if (error?.name !== 'AbortError') showMemeNotice('Image ready — use the share sheet to save it.', 'info');
      });
    };
    document.addEventListener('click', intercept, true);
    memeShareSaveBridgeInstalled = true;
  };

  const installMemeCloseAccessibility = () => {
    const overlay = document.querySelector('div[class~="fixed"][class~="inset-0"][class~="z-[9999]"][class~="bg-black/95"]');
    const close = overlay && overlay.querySelector('[data-loc="client/src/pages/Home.tsx:567"]');
    if (!close) return;
    close.setAttribute('aria-label', 'Close meme');
    close.setAttribute('title', 'Close');
    close.setAttribute('data-tb-meme-close', '1');
  };
  ...
  const directPhotoTimer = window.setInterval(() => {
    ...
    installMemeCloseAccessibility();
    installMemeShareSaveBridge();
    applyReaderAudioPolish();
    ensureProfilePhotoNudgeGuard();
  ...
  try { installMemeCloseAccessibility(); } catch (_) {}
  try { installMemeShareSaveBridge(); } catch (_) {}

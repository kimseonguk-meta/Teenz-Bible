(() => {
  ...
  const upsertCenteredHeaderTitle = (container, pageName, className) => {
    ...
    title.innerHTML = '<span class="tb-centered-header-title__name"></span>';
  ...
  const simplifyBibleAiHeader = () => {
    ...
    title.innerHTML = '<span class="tb-ai-shared-title__name">Bible AI</span>';
  };
  ...
  const normalizeCrewModalViewport = () => {
    ...
    Array.from(document.querySelectorAll('.fixed.inset-0')).forEach((overlay) => {
      if (!crewModalPattern.test(overlay.innerText || '')) return;
      overlay.classList.add('tb-crew-modal-viewport');
  ...
  const applyCenteredHeaders = () => {
    ...
    if (biblePage && !biblePage.querySelector(':scope > .tb-centered-page-header')) {
      ...
      title.innerHTML = '<span class="tb-centered-page-header__name">Bible</span>';

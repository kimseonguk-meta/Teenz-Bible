(() => {
  ...
  // The old Ranking CTA was only an href to /profile. Carry an explicit intent
  // across navigation so Profile opens its existing crew composer immediately.
  const CREW_COMPOSER_INTENT_KEY = 'teenzBibleOpenCrewComposer';
  const openPendingCrewComposer = () => {
    if (window.location.pathname !== '/profile') return;
    const composerRequested = localStorage.getItem(CREW_COMPOSER_INTENT_KEY) === 'create' || new URLSearchParams(window.location.search).get('crew') === 'create';
    if (!composerRequested) return;
    localStorage.setItem(CREW_COMPOSER_INTENT_KEY, 'create');
    const startedAt = Number(sessionStorage.getItem(`${CREW_COMPOSER_INTENT_KEY}:startedAt`) || Date.now());
    sessionStorage.setItem(`${CREW_COMPOSER_INTENT_KEY}:startedAt`, String(startedAt));
    if (Date.now() - startedAt > 10000) {
      localStorage.removeItem(CREW_COMPOSER_INTENT_KEY);
      sessionStorage.removeItem(`${CREW_COMPOSER_INTENT_KEY}:startedAt`);
      return;
    }
    const create = Array.from(document.querySelectorAll('button')).find((candidate) => /^(?:➕\s*)?create crew$/i.test(candidate.innerText.trim()));
    if (create) {
      localStorage.removeItem(CREW_COMPOSER_INTENT_KEY);
      sessionStorage.removeItem(`${CREW_COMPOSER_INTENT_KEY}:startedAt`);
    ...
    const chooser = Array.from(document.querySelectorAll('button')).find((candidate) => /join\s+or\s+create\s+a\s+crew/i.test(candidate.innerText.trim()));
    if (chooser) chooser.click();
    window.setTimeout(openPendingCrewComposer, chooser ? 80 : 250);
  };
  ...
  const installCrewComposerEntryPoint = () => {
    ...
    createJoin.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      localStorage.setItem(CREW_COMPOSER_INTENT_KEY, 'create');
      sessionStorage.removeItem(`${CREW_COMPOSER_INTENT_KEY}:startedAt`);
      window.location.assign('/profile?crew=create');
    }, true);
  };

(() => {
  ...
  const openPendingCrewComposer = () => {
    ...
    if (localStorage.getItem(CREW_COMPOSER_INTENT_KEY) !== 'create') return;
    ...
    if (!chooser) return;
    chooser.click();
    window.setTimeout(openPendingCrewComposer, 80);
  };

  const installCrewComposerEntryPoint = () => {
    const createJoin = document.querySelector('[data-loc="client/src/pages/Leaderboard.tsx:962"]');
    if (!createJoin || createJoin.dataset.tbCrewComposerPatched === 'true') return;
    createJoin.dataset.tbCrewComposerPatched = 'true';
    createJoin.textContent = '👥 Create / Join Crew';

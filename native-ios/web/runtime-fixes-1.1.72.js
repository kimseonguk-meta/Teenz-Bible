(() => {
  ...
  const openPendingCrewComposer = () => {
    ...
    const button = Array.from(document.querySelectorAll('button')).find((candidate) => /join\s+or\s+create\s+a\s+crew/i.test(candidate.innerText.trim()));
    if (!button) return;
